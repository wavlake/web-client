...
async function main() {
  ...
  if (quote.paid) {
    ...
    console.log('Minted:', minted, 'USD cents');
    console.log('Keyset IDs:', wallet.proofs.map(p => p.id));
    
    const token = getEncodedTokenV4({ mint: MINT_URL, proofs: wallet.proofs, unit: 'usd' });
    console.log('\n✅ USD TOKEN:\n' + token);
  } else {
